...
layer make_batchnorm_layer(int batch, int w, int h, int c)
{
    ...
    l.output_gpu =  cl_make_array(l.output, h * w * c * batch);
    l.delta_gpu =   cl_make_array(l.delta, h * w * c * batch);

    l.biases_gpu = cl_make_array(l.biases, c);
    l.bias_updates_gpu = cl_make_array(l.bias_updates, c);

    l.scales_gpu = cl_make_array(l.scales, c);
    l.scale_updates_gpu = cl_make_array(l.scale_updates, c);

    l.mean_gpu = cl_make_array(l.mean, c);
    l.variance_gpu = cl_make_array(l.variance, c);

    l.rolling_mean_gpu = cl_make_array(l.mean, c);
    l.rolling_variance_gpu = cl_make_array(l.variance, c);

    l.mean_delta_gpu = cl_make_array(l.mean, c);
    l.variance_delta_gpu = cl_make_array(l.variance, c);
...
{
    cl_pull_array(l.scales_gpu, l.scales, l.c);
	cl_pull_array(l.rolling_mean_gpu, l.rolling_mean, l.c);
	cl_pull_array(l.rolling_variance_gpu, l.rolling_variance, l.c);
}
void push_batchnorm_layer(layer l)
{
    cl_push_array(l.scales_gpu, l.scales, l.c);
	cl_push_array(l.rolling_mean_gpu, l.rolling_mean, l.c);
	cl_push_array(l.rolling_variance_gpu, l.rolling_variance, l.c);
}
