...
cost_layer make_cost_layer(int batch, int inputs, COST_TYPE cost_type, float scale)
{
    ...
    l.delta_gpu = cl_make_array(l.output, inputs*batch);
    l.output_gpu = cl_make_array(l.delta, inputs*batch);
    #endif
    return l;
}
...
void resize_cost_layer(cost_layer *l, int inputs)
{
    ...
    l->delta_gpu = cl_make_array(l->delta, inputs*l->batch);
    l->output_gpu = cl_make_array(l->output, inputs*l->batch);
#endif
}
...
{
    cl_pull_array(l.delta_gpu, l.delta, l.batch*l.inputs);
}
...
void forward_cost_layer_gpu(cost_layer l, network net)
{
    ...
    cl_pull_array(l.output_gpu, l.output, l.batch*l.inputs);
    l.cost[0] = sum_array(l.output, l.batch*l.inputs);
}
