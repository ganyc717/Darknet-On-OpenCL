...
layer make_lstm_layer(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    *(l.wo) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.output_gpu = cl_make_array(0, batch*outputs*steps);
    l.delta_gpu = cl_make_array(0, batch*l.outputs*steps);

    l.prev_state_gpu = cl_make_array(0, batch*outputs);
    l.prev_cell_gpu = cl_make_array(0, batch*outputs);
    l.cell_gpu = cl_make_array(0, batch*outputs*steps);

    l.f_gpu = cl_make_array(0, batch*outputs);
    l.i_gpu = cl_make_array(0, batch*outputs);
    l.g_gpu = cl_make_array(0, batch*outputs);
    l.o_gpu = cl_make_array(0, batch*outputs);
    l.c_gpu = cl_make_array(0, batch*outputs);
    l.h_gpu = cl_make_array(0, batch*outputs);
    l.temp_gpu = cl_make_array(0, batch*outputs);
    l.temp2_gpu = cl_make_array(0, batch*outputs);
    l.temp3_gpu = cl_make_array(0, batch*outputs);
    l.dc_gpu = cl_make_array(0, batch*outputs);
    l.dh_gpu = cl_make_array(0, batch*outputs);
#ifdef CUDNN
...
void backward_lstm_layer_gpu(layer l, network state)
{
    ...
    for (i = l.steps - 1; i >= 0; --i) {
        ...
        l.dh_gpu = (i == 0) ? CLArray() : l.delta_gpu - l.outputs*l.batch;
