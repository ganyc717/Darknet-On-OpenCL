	...
	CLArray C_gpu, int ldc)
{
	...
	check_error(clEnqueueSVMMap(*(cl->queue), CL_TRUE, CL_MAP_WRITE, A_gpu.buffer, A_gpu.size * sizeof(float), 0, NULL, NULL));
	check_error(clEnqueueSVMMap(*(cl->queue), CL_TRUE, CL_MAP_WRITE, B_gpu.buffer, B_gpu.size * sizeof(float), 0, NULL, NULL));
	check_error(clEnqueueSVMMap(*(cl->queue), CL_TRUE, CL_MAP_WRITE, C_gpu.buffer, C_gpu.size * sizeof(float), 0, NULL, NULL));
	cl_mem A = clCreateBuffer(*(cl->context), CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, A_gpu.size * sizeof(float), A_gpu.buffer, &status);
	cl_mem B = clCreateBuffer(*(cl->context), CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, B_gpu.size * sizeof(float), B_gpu.buffer, &status);
	cl_mem C = clCreateBuffer(*(cl->context), CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, C_gpu.size * sizeof(float), C_gpu.buffer, &status);
	...
	cl_event events[3];
	clEnqueueSVMUnmap(*(cl->queue), A_gpu.buffer, 0, NULL, &events[0]);
	clEnqueueSVMUnmap(*(cl->queue), B_gpu.buffer, 0, NULL, &events[1]);
	clEnqueueSVMUnmap(*(cl->queue), C_gpu.buffer, 0, NULL, &events[2]);
