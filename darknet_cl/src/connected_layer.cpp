...
layer make_connected_layer(int batch, int inputs, int outputs, ACTIVATION activation, int batch_normalize, int adam)
{
    ...
    l.weights_gpu = cl_make_array(l.weights, outputs*inputs);
    l.biases_gpu = cl_make_array(l.biases, outputs);

    l.weight_updates_gpu = cl_make_array(l.weight_updates, outputs*inputs);
    l.bias_updates_gpu = cl_make_array(l.bias_updates, outputs);

    l.output_gpu = cl_make_array(l.output, outputs*batch);
    l.delta_gpu = cl_make_array(l.delta, outputs*batch);
    if (adam) {
        l.m_gpu = cl_make_array(0, inputs*outputs);
        l.v_gpu = cl_make_array(0, inputs*outputs);
        l.bias_m_gpu = cl_make_array(0, outputs);
        l.bias_v_gpu = cl_make_array(0, outputs);
        l.scale_m_gpu = cl_make_array(0, outputs);
        l.scale_v_gpu = cl_make_array(0, outputs);
    }

    if(batch_normalize){
        l.mean_gpu = cl_make_array(l.mean, outputs);
        l.variance_gpu = cl_make_array(l.variance, outputs);

        l.rolling_mean_gpu = cl_make_array(l.mean, outputs);
        l.rolling_variance_gpu = cl_make_array(l.variance, outputs);

        l.mean_delta_gpu = cl_make_array(l.mean, outputs);
        l.variance_delta_gpu = cl_make_array(l.variance, outputs);

        l.scales_gpu = cl_make_array(l.scales, outputs);
        l.scale_updates_gpu = cl_make_array(l.scale_updates, outputs);
...
{
    cl_pull_array(l.weights_gpu, l.weights, l.inputs*l.outputs);
	cl_pull_array(l.biases_gpu, l.biases, l.outputs);
	cl_pull_array(l.weight_updates_gpu, l.weight_updates, l.inputs*l.outputs);
	cl_pull_array(l.bias_updates_gpu, l.bias_updates, l.outputs);
    if (l.batch_normalize){
		cl_pull_array(l.scales_gpu, l.scales, l.outputs);
		cl_pull_array(l.rolling_mean_gpu, l.rolling_mean, l.outputs);
		cl_pull_array(l.rolling_variance_gpu, l.rolling_variance, l.outputs);
    }
}

void push_connected_layer(layer l)
{
    cl_push_array(l.weights_gpu, l.weights, l.inputs*l.outputs);
	cl_push_array(l.biases_gpu, l.biases, l.outputs);
	cl_push_array(l.weight_updates_gpu, l.weight_updates, l.inputs*l.outputs);
	cl_push_array(l.bias_updates_gpu, l.bias_updates, l.outputs);
    if (l.batch_normalize){
		cl_push_array(l.scales_gpu, l.scales, l.outputs);
		cl_push_array(l.rolling_mean_gpu, l.rolling_mean, l.outputs);
		cl_push_array(l.rolling_variance_gpu, l.rolling_variance, l.outputs);
    }
}
...
void forward_connected_layer_gpu(layer l, network net)
{
    ...
    gemm_gpu(0,1,m,n,k,1, net.input_gpu,k, l.weights_gpu,k,1, l.output_gpu,n);
...
void backward_connected_layer_gpu(layer l, network net)
{
    ...
    gemm_gpu(1,0,m,n,k,1, l.delta_gpu,m, net.input_gpu,n,1, l.weight_updates_gpu,n);
