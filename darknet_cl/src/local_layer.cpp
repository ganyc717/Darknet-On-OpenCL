...
void forward_local_layer_gpu(const local_layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        CLArray input = net.input_gpu + i*l.w*l.h*l.c;
        im2col_gpu(input, l.c, l.h, l.w, 
                l.size, l.stride, l.pad, net.workspace_gpu);
		CLArray output = l.output_gpu + i*l.outputs;
        for(j = 0; j < locations; ++j){
			CLArray a = l.weights_gpu + j*l.size*l.size*l.c*l.n;
			CLArray b = net.workspace_gpu + j;
			CLArray c = output + j;
...
void backward_local_layer_gpu(local_layer l, network net)
{
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        for(j = 0; j < locations; ++j){ 
            CLArray a = l.delta_gpu + i*l.outputs + j;
			CLArray b = net.workspace_gpu + j;
			CLArray c = l.weight_updates_gpu + j*l.size*l.size*l.c*l.n;
            int m = l.n;
            int n = l.size*l.size*l.c;
            int k = 1;
...
void pull_local_layer(local_layer l)
{
    int locations = l.out_w*l.out_h;
    int size = l.size*l.size*l.c*l.n*locations;
    cl_pull_array(l.weights_gpu, l.weights, size);
